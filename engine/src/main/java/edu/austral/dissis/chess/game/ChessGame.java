...
public class ChessGame extends Game {
  ...
  private Move normalizeMove(Move move) {
    if (move instanceof ChessMove cm && cm.type() != ChessMoveType.STANDARD) return move;
    return currentState().board().pieceAt(move.from())
            .map(p -> normalizeByPieceType(move, p))
            .orElse(move);
  }

  private Move normalizeByPieceType(Move move, Piece piece) {
    if (piece.isType(PieceType.KING)) return detectCastling(move, piece);
    if (piece.isType(PieceType.PAWN)) return detectEnPassant(move);
    return move;
  }

  private Move detectCastling(Move move, Piece piece) {
    int expectedRow = piece.color() == Color.WHITE ? 0 : 7;
    if (!isKingOnStartSquare(move.from(), expectedRow)) return move;
    if (move.to().row() != move.from().row()) return move;
  ...
  private Move castlingMoveFor(Move move, int colDelta) {
    if (colDelta == 2) return ChessMove.castlingKingside(move.from(), move.to());
    if (colDelta == -2) return ChessMove.castlingQueenside(move.from(), move.to());
    return move;
  }
  ...
  private Move detectEnPassant(Move move) {
    ...
    if (!(currentState().extra() instanceof ChessExtra extra)) return move;

package edu.austral.dissis.checkers.rules.win;
...
public class CheckersWinCondition implements WinCondition {
    ...
    public CheckersWinCondition(MoveValidator moveValidator) {
    ...
    public GameStatus evaluate(GameState state) {
        ...
        if (!hasAnyLegalMove(current, state)) {
    ...
    private boolean pieceHasLegalMove(Position from, GameState state) {
        return allPositions(state)
                .anyMatch(to -> moveValidator.findViolation(new Move(from, to), state).isEmpty());
    }

...
fun main() {
    launch(CheckersServerApplication::class.java)
}

class CheckersServerApplication : Application() {
    private val imageResolver = CachedImageResolver(DefaultImageResolver())

    override fun start(primaryStage: Stage) {
        val serverState =
            GameServerState(CheckersGameFactory.createStandardGame()) { from, to ->
                CheckersMove(from, to)
            }

        primaryStage.title = "Checkers - Servidor (Blancas)"
